# UR10 预实验工程 — 本轮版本说明（XY 微动 + 视觉闭环快速测试）

本包是 `UR10_vibration_pretest` 的源码交付包，供人工或其它模型复核本轮改动。
根目录扁平，全部源码在根下，单测在 `tests/`，**不含任何输出数据与图片**。

> **包版本：`V1c`（2026-09-17）。**
> 相对 `V1b` 的三处定向修正：**① 符号只转换一次**（控制律不再接 `sign`，
> 修掉 sign=−1 安装下闭环发散的真实缺陷）；**② 闭环目标预算 6 → 12**
> （原估算漏了方向这一维）；**③ 删除「到 10 分钟强制收尾」策略**
> （时长只提示、不中止）。**测试数 182 → 204**。
> 全部改动见文末「§13 V1b → V1c 改动清单」。
> 若你手上是更早的包，请一律以本包为准。

## 版本说明（本轮做了什么）

上一轮的定位是"标定 UR10 从 5 到 200 μm 的最小分辨率曲线"：11 个档位的开环扫描
（`micro_motion_experiment`，已删除），实测运行时间 2.7～5 小时。

**本轮把规模与运行时间整体收缩**，定位改成：

> **约 10～12 分钟，快速判断 UR10 CB3 在 X/Y 两个方向上、5 / 20 / 50 μm 三个数量级
> 的开环微动能力与视觉闭环逼近能力。**
>
> （时长只是提示：跑完 12 个闭环目标优先于压时间，**没有强制截止**。）

因此**不是**增加档位、**不是**自适应搜索、**不是**长时间离线分析。具体：

| | 上一轮 `micro_motion_experiment`（已删除） | 本轮 |
|---|---|---|
| 档位 | 11 档（200…5 μm）扫描 | **固定 3 档：5 / 20 / 50 μm，顺序固定从小到大** |
| 模式 | 只有开环单步 | 开环连续 / 开环来回 / 视觉闭环 |
| 重复 | 每档 10～20 次 | 每块 6 步，**不做 10/20 次重复** |
| 运行时间 | 2.7～5 小时 | **正常约 11.3 分钟，无强制截止** |
| 输出 | 大量离线图与长视频 | 逐步 CSV + JSON + 少量 PNG 证据帧 |
| 结论 | "最小分辨率 = XX μm" | "X 5 μm：… / X 20 μm：… / X 50 μm：…" |

原始 RAW 不再写进工程目录，改写到 `D:/UR10_micro_raw/<run_id>/<block_id>/`，
按实验块分目录，**本轮全部保留**（磁盘 277 GiB 余量足够）。
...
4. **开环和视觉闭环能力差多少**？
5. X / Y 是否存在明显差异？
6. 高频繁换向（模拟未来 SFC 的高频正负修正）是否明显劣于连续同向？

**不回答**"最小微动究竟是 27 还是 31 μm"——那需要扫更多档位，正是本轮刻意不做的。
所以最终报告里不会出现"UR10 最小分辨率 = XX μm"这种结论。
...
D:/UR10_micro_raw/<run_id>/                        ← 原始 RAW（全量保留）
├─ 00_static/          ← 静止基线，永久保留
├─ 01_probe/
├─ 02_open_baseline/
...
临时 RAW 只存在于工程内的 `outputs/_micro_temp/`，用完即归档到 D:（同卷
`os.replace`，原子且瞬间完成）或移入 `outputs/_micro_trash/`。
`00_static` 块**永久保留**，它是"取消 MJPG 之后静止抖动到底是多少"的唯一原始证据。
另外全部微动块按 `MICRO_LOOP_KEEP_ALL_RAW = True` 本轮全量保留，
**绝不静默删除**。启动时会按**最坏**数据量（154.0 GiB）检查磁盘并预留 15 GiB 余量，
不满足就**在创建任何目录之前**拒绝开始。
...
```python
def send_cartesian_micro_correction(robot, axis, delta_um, *, stop_event, write_state,
                                    settle_timeout_s=None) -> dict
```
...
`VISION_METHOD == "checkerboard"`、D: 剩余 ≥ 最坏数据量 + 15 GiB、
可用物理内存 ≥ 1.5 GiB。
...
[状态] 静止噪声底：sigma 0.61 μm ｜ RMS 0.74 μm ｜ 完整 RAW 已永久保留在 D:\…
...
[状态] 结果目录：… ／ 原始 RAW 目录：D:\UR10_micro_raw\micro_motion_…
```
...
| 项目 | 结果 |
|---|---|
| `python -m unittest discover -s tests` | **204 个用例全绿** |
