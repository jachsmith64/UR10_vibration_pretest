...
def build_template(d: dict[str, Any], duration_s: float = config.TEMPLATE_DURATION_S,
    ...
    e_source：本层要用的误差序列。默认取原始 cross_um（上一轮口径）；
              本轮传 drift 扣掉慢偏移后的 e_vib，模板与沿程 schedule 完全不变，
              只把「被重建的那条曲线」换掉。
    """
    t, al = d["t"], d["along_mm"]
...
PLANT_FILENAME = "plant_frf.npz"     # 标定结果落盘，供测试B 的多次重建复用


def save_plant(plant: Plant, path: Path) -> Path:
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    np.savez(str(path), freq_hz=plant.freq_hz,
             gain_re=plant.gain.real, gain_im=plant.gain.imag,
             dc_um_per_n=np.array([plant.dc_um_per_n]))
    return path


def load_plant(path: Path) -> Plant | None:
    """读回已标定的 Plant。读不到返回 None（调用方自行决定是否重测）。"""
    path = Path(path)
    if not path.is_file():
        return None
    z = np.load(str(path))
    return Plant(freq_hz=z["freq_hz"],
                 gain=z["gain_re"] + 1j * z["gain_im"],
                 dc_um_per_n=float(z["dc_um_per_n"][0]))
...
@dataclass
class Prepared:
    """重建前的全部前置量：读数据 → 重采样 → 切窗 → 构造 b(t)/e_vib。

    单独抽出来是为了让「同一份数据 + 同一个 b(t)，只换 Δb」的测试B
    不必重复读盘与重复构造基线，也不必把 Plant 与 q_traj 重测一遍。
    """
    raw: dict[str, Any]
    d: dict[str, Any]
    cuts: Cuts
...
def prepare(log: Callable[[str], None] = print) -> Prepared:
    """读预实验数据并构造理想慢偏移 b(t) 与净振动 e_vib（不含任何仿真）。"""
    ...
    dr = drift.build(d, cuts, log=log)
    return Prepared(raw=raw, d=d, cuts=cuts, drift=dr)
...
def reconstruct_fd(out_dir: Path, duration_s: float = config.TEMPLATE_DURATION_S,
    ...
    """执行重建全流程并冻结 Fd。返回 meta。

    口径固定为 e_vib（§六）：先构造并扣除理想慢偏移 b(t)，只对净振动做反演与迭代。
    不再提供「直接用 e_raw 重建」的入口 —— 那是被本轮废止的旧口径。

    bias_um ≠ 0 时进入测试B：b̂ = b + Δb，用 e_vib^err = e_raw − b̂ 重建，
    用来回答「慢偏移估计有小误差时 SFC 是否还成立」。

    prepared / plant / q_traj 可外部传入以复用（测试B 连做 4 次重建时用）。
    """
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    pr = prepared if prepared is not None else prepare(log=log)
    raw, d, dr = pr.raw, pr.d, pr.drift
    cuts = pr.cuts
    e_raw = np.asarray(dr["e_raw_um"], dtype=float)
    b_true = np.asarray(dr["b_um"], dtype=float)

    if float(bias_um) != 0.0:
        b_hat = b_true + float(bias_um)
        e_vib = e_raw - b_hat
        log(f"[重建] 测试B：慢偏移含误差 Δb = {bias_um:+.1f} µm，"
            f"用 e_vib^err = e_raw − b̂ 重建")
    else:
        b_hat = b_true
        e_vib = np.asarray(dr["e_vib_um"], dtype=float)
    log(f"[重建] e_vib rms {np.sqrt(np.mean(e_vib ** 2)):.3f} µm"
        f"（e_raw rms {np.sqrt(np.mean(e_raw ** 2)):.3f} µm）")

    if write_references:
        drift.write_reference_csvs(out_dir, dr, d, log=log)
        drift.write_summary_json(out_dir / config.DRIFT_SUMMARY_JSON, dr, log=log)

    T = build_template(d, duration_s=duration_s, e_source=e_vib)
    # 三件套的【模板网格】版本。build_template 的分段只由 d 决定（与 e_source 无关），
    # 所以用同一个 d / duration_s 再建两次，得到的就是同一套时间轴上的 e_raw(t) 与 b̂(t)。
    # 时序 CSV 要带的参考列必须是模板网格（62.85 s）而不是记录网格（7.755 s），
    # 否则与仿真量对不齐。
    ref_tpl: dict[str, np.ndarray] = {
        "e_raw_um": build_template(d, duration_s=duration_s, e_source=e_raw).e_um,
        "b_um": build_template(d, duration_s=duration_s, e_source=b_hat).e_um,
        "e_vib_um": T.e_um,
    }
    if any(v.shape != T.e_um.shape for v in ref_tpl.values()):
        raise ValueError("[重建] 参考列的模板网格长度不一致，无法作为时序参考列")
    ...
    if plant is None:
        ...
        if write_references:
            save_plant(plant, out_dir / PLANT_FILENAME)
            log(f"[重建] Plant 频响已落盘 {PLANT_FILENAME}（供测试B 复用同一份标定）")
    else:
        log("[重建] 复用已标定的 Plant 频响（测试B 不重复标定）")

    if q_traj is None:
        log("[重建] 名义关节轨迹 IK…")
        q_traj = build_nominal_joint_trajectory(T.along_mm, config.Y_NOMINAL,
                                                config.Z_HOLD)
    else:
        q_traj = np.asarray(q_traj, dtype=float)
        if q_traj.shape != (T.along_mm.size, q_traj.shape[-1]):
            raise ValueError(f"[重建] 传入的 q_traj 形状 {q_traj.shape} 与模板 "
                             f"{T.along_mm.size} 点不符")
    ...
    meta = {
        ...
        "recon_source": "vib",
        "source_note": "先扣除理想慢偏移 b(t)，只重建净振动 e_vib（§六 唯一口径）",
        "data_id": "P05R01",
        ...
        "drift_baseline": {
            "method": dr["method"],
            "bias_um": float(bias_um),
            "is_bias_test": bool(float(bias_um) != 0.0),
            "levels_um": {
                "b_pre": dr["levels"]["b_pre_um"], "b_out": dr["levels"]["b_out_um"],
                "b_ret": dr["levels"]["b_ret_um"], "b_post": dr["levels"]["b_post_um"],
            },
            "windows": dr["levels"]["windows"],
            "window_sensitivity": dr["levels"]["window_sensitivity"],
            "transitions": dr["transitions"],
            "b_range_um": [float(dr["b_um"].min()), float(dr["b_um"].max())],
            "steady_segment_checks": dr["steady_checks"],
            "overall": dr["overall"],
            "post_stop_stationarity": dr["post_stop_stationarity"],
            "acceptance": {"passed": bool(dr["passed"]), "checks": dr["checks"]},
            "notes": dr["notes"],
            "files": {"raw_reference": config.DRIFT_RAW_CSV,
                      "drift_baseline": config.DRIFT_BASELINE_CSV,
                      "vibration_reference": config.DRIFT_VIB_CSV,
                      "raw_plus_baseline_table": config.DRIFT_TABLE_CSV,
                      "drift_baseline_summary": config.DRIFT_SUMMARY_JSON},
        ...
        "template": {
            ...
            # 分段区间是【左闭右开】的，直接交给 record.phase_codes_from_segments
            # 生成时序里的 phase 列；分段只由 d 决定，与 Δb 无关。
            "segments": [[name, int(i0), int(i1)] for name, i0, i1 in T.segments],
            "phase_names": list(config.DRIFT_PHASE_NAMES),
    ...
    record.save_json(out_dir / config.FD_META_FILENAME, meta)
