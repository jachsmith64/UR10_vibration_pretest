...
TRACE_COLUMNS = (["t", "phase", "along_mm", "y_d", "y_sim", "e_um",
...
# 单位说明，写进文件头，避免下游猜
UNITS = {
    "t": "s", "phase": "code", "along_mm": "mm", "y_d": "m", "y_sim": "m",
    "e_um": "um", "e_raw_um": "um", "b_um": "um", "e_vib_um": "um",
...
def phase_codes_from_segments(segments: list[tuple[str, int, int]]) -> np.ndarray:
    """把 build_template 的分段列表映射成 5 个运动阶段的整数编码。

    注意区间口径：build_template 的 segments 是【左闭右开】的 (name, i0, i1)，
    直接照搬，不要 +1/−1 换算。

    编码 → 名称的对照写在文件头 `# phase_names=...` 一行里（见 write_trace），
    这样 phase 列可以留在纯数值块内，不破坏 load_trace 的 np.loadtxt 解析。
    """
    if not segments:
        raise ValueError("[记录] 分段列表为空，无法生成 phase 列")
    n = max(int(i1) for _, _, i1 in segments)
    code = np.full(n, -1, dtype=np.int64)
    for name, i0, i1 in segments:
        phase = config.DRIFT_TEMPLATE_TO_PHASE.get(name)
        if phase is None:
            raise ValueError(f"[记录] 未知的模板分段名 {name!r}，"
                             f"请同步更新 config.DRIFT_TEMPLATE_TO_PHASE")
        code[int(i0):int(i1)] = config.DRIFT_PHASE_CODE[phase]
    if np.any(code < 0):
        miss = np.where(code < 0)[0]
        raise ValueError(f"[记录] 模板分段未能覆盖全部 {n} 点，"
                         f"缺失 {miss.size} 点（首个 idx={int(miss[0])}）")
    return code

# e_raw/b/e_vib 这三列【不是仿真量】，而是预实验参考序列在同一时间索引上的回放：
#   e_raw_um = 预实验原始横向误差（含慢偏移）
#   b_um     = 构造的理想慢偏移基线（见 drift.py）
...
def write_trace(path: Path, data: dict[str, np.ndarray],
    ...
    with open(path, "w", encoding="utf-8", newline="") as fh:
        ...
        # phase 列的取值对照表。写成一行逗号分隔，load_trace 会把它收进 meta
        # （含 "=" 的非 trace/units 注释行），而 phase 列本身仍是纯整数，
        # 不破坏 np.loadtxt 的纯数值解析。
        fh.write("# phase_names=" + ",".join(config.DRIFT_PHASE_NAMES) + "\n")
...
def json_default(o: Any):
    """把 numpy 标量还原成 JSON 原生类型。

    这里刻意【不】用 str 兜底处理 numpy：np.bool_(True) 一旦被 str 成 "True"，
    下游 `if not v` 的验收判据会把字符串当真值，判定全部失效。宁可在这里
    显式转换，也不要静默降级成字符串。
    """
    ...
    if isinstance(o, Path):
        return str(o)
    raise TypeError(f"JSON 无法序列化 {type(o).__name__}：{o!r}")
